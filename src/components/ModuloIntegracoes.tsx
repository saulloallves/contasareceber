...
import { 
  Settings, Zap, MessageSquare, Mail, Database, Globe, 
  CheckCircle, XCircle, AlertTriangle, RefreshCw, Eye, 
  Edit, TestTube, Download, Upload, Play, Pause, Clock,
  BarChart3, TrendingUp, Activity, Wifi, WifiOff, 
  Shield, Key, Link, Send, FileText, Filter
} from 'lucide-react';
...
export function ModuloIntegracoes() {
  const [abaSelecionada, setAbaSelecionada] = useState<'configuracoes' | 'monitoramento' | 'historico' | 'estatisticas'>('configuracoes');
  ...
  const [estatisticas, setEstatisticas] = useState<EstatisticasIntegracoes | null>(null);
  const [carregando, setCarregando] = useState(true);
  const [modalAberto, setModalAberto] = useState<'configurar' | 'testar' | 'historico' | 'reenviar' | null>(null);
  ...
  const carregarDados = async () => {
    setCarregando(true);
    try {
      const [integracoesData, monitoramentoData, estatisticasData] = await Promise.all([
      ...
      if (abaSelecionada === 'historico') {
        const historicoData = await integracoesService.buscarHistoricoIntegracoes(filtros);
  ...
  const testarConexao = async (integracao: IntegracaoConfig) => {
    ...
    try {
      ...
      alert(`Teste ${resultado.sucesso ? 'bem-sucedido' : 'falhou'}: ${resultado.detalhes}`);
      carregarDados();
    } catch (error) {
  ...
  const exportarConfiguracoes = async () => {
    try {
      ...
      a.href = url;
      a.download = `integracoes-config-${new Date().toISOString().split('T')[0]}.json`;
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Módulo de Integrações</h1>
              <p className="text-gray-600">Comunicação e orquestração externa</p>
            </div>
          </div>
        ...
        {estatisticas && (
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-600">{estatisticas.total_integracoes}</div>
              <div className="text-sm text-blue-800">Total de Integrações</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-green-600">{estatisticas.integracoes_ativas}</div>
              <div className="text-sm text-green-800">Integrações Ativas</div>
            </div>
            <div className="bg-purple-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-purple-600">{estatisticas.chamadas_24h}</div>
              <div className="text-sm text-purple-800">Chamadas 24h</div>
            </div>
            <div className="bg-yellow-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-yellow-600">{estatisticas.taxa_sucesso_geral.toFixed(1)}%</div>
              <div className="text-sm text-yellow-800">Taxa de Sucesso</div>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-gray-600">{formatarTempo(estatisticas.tempo_resposta_medio)}</div>
        ...
        {abaSelecionada === 'configuracoes' && (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800">Configurações de Integrações</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {integracoes.map((integracao) => (
                <div key={integracao.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      {getTipoIcon(integracao.tipo)}
                      <div className="ml-3">
                        <h4 className="text-lg font-semibold text-gray-800">{integracao.nome}</h4>
                        <p className="text-sm text-gray-600">{integracao.tipo.toUpperCase()}</p>
                      </div>
                    </div>
                    <div className="flex items-center">
                      {getStatusIcon(integracao.status_conexao)}
                      <span className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(integracao.status_conexao)}`}>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={integracao.ativo ? 'text-green-600' : 'text-red-600'}>
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-gray-800">
                        {integracao.ultima_verificacao ? formatarData(integracao.ultima_verificacao) : 'Nunca'}
                      </span>
                    </div>
                  </div>
                  ...
                  {monitoramento.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{item.integracao}</div>
                        <div className="text-sm text-gray-500">
                          Última atividade: {formatarData(item.ultima_atividade)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          {item.status === 'online' ? <Wifi className="w-5 h-5 text-green-600 mr-2" /> : <WifiOff className="w-5 h-5 text-red-600 mr-2" />}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-medium">{item.taxa_sucesso_24h.toFixed(1)}%</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        ...
                        {item.alertas_ativos.length > 0 ? (
                          <div className="space-y-1">
                            {item.alertas_ativos.map((alerta, i) => (
                              <span key={i} className="inline-block px-2 py-1 text-xs bg-red-100 text-red-800 rounded-full">
                                {alerta}
                              </span>
                            ))}
                          </div>
                        ) : (
                          <span className="text-green-600 text-sm">Sem alertas</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        {abaSelecionada === 'historico' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-800">Histórico de Execuções</h3>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {historico.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          {getTipoIcon((item as any).integracoes_config?.tipo || 'webhook')}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.tempo_resposta ? formatarTempo(item.tempo_resposta) : '-'}
                      </td>
        ...
        {abaSelecionada === 'estatisticas' && estatisticas && (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800">Estatísticas Detalhadas</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h4 className="font-semibold text-gray-800 mb-4">Performance por Tipo</h4>
                <div className="space-y-4">
                  {Object.entries(estatisticas.por_tipo).map(([tipo, stats]) => (
                    <div key={tipo} className="flex items-center justify-between">
                      <div className="flex items-center">
                        {getTipoIcon(tipo)}
                        <span className="ml-2 font-medium text-gray-800">{tipo.toUpperCase()}</span>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-medium text-gray-900">{stats.sucesso.toFixed(1)}%</div>
                        <div className="text-xs text-gray-500">{stats.chamadas} chamadas</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{estatisticas.total_integracoes}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Integrações Ativas:</span>
                    <span className="font-medium text-green-600">{estatisticas.integracoes_ativas}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Chamadas 24h:</span>
                    <span className="font-medium">{estatisticas.chamadas_24h}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Taxa de Sucesso:</span>
                    <span className="font-medium text-green-600">{estatisticas.taxa_sucesso_geral.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Tempo Médio:</span>
                    <span className="font-medium">{formatarTempo(estatisticas.tempo_resposta_medio)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {integracaoSelecionada.tipo === 'whatsapp' && (
                <>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">ID do Remetente</label>
                    <input
                      type="text"
                      value={formData.id_remetente || ''}
                      onChange={(e) => setFormData({...formData, id_remetente: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      placeholder="Phone Number ID ou Instance ID"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Número Autenticado</label>
                    <input
                      type="text"
                      value={formData.numero_autenticado || ''}
                      onChange={(e) => setFormData({...formData, numero_autenticado: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({...formData, servidor_smtp: e.target.value})}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({...formData, token_autenticacao: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({...formData, token_integracao: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      placeholder="secret_..."
                    />
                  </div>
