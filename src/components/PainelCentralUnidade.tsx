...
import { 
  Building2, DollarSign, Calendar, MessageSquare, FileText, Users, 
  MapPin, Phone, Mail, Instagram, Edit, Save, X, Plus, Download,
  TrendingUp, TrendingDown, AlertTriangle, CheckCircle, Clock,
  Filter, Search, Eye, Send, Bell, Scale, RefreshCw
} from 'lucide-react';
import { UnidadeCentralService } from '../services/unidadeCentralService';
import { UnidadeCentral, CobrancaUnidade, ReuniaoUnidade, ComunicacaoUnidade, DashboardUnidade, VinculoFranqueado, FiltrosUnidadeCentral } from '../types/unidadeCentral';
...
export function PainelCentralUnidade() {
  ...
  const [abaSelecionada, setAbaSelecionada] = useState<'dados' | 'cobrancas' | 'agenda' | 'comunicacoes' | 'dashboard' | 'vinculos'>('dados');
  const [editandoDados, setEditandoDados] = useState(false);
  const [dadosEdicao, setDadosEdicao] = useState<Partial<UnidadeCentral>>({});
  const [modalAberto, setModalAberto] = useState<'reuniao' | 'mensagem' | 'notificacao' | 'escalonamento' | null>(null);
  ...
  const buscarUnidade = async () => {
    ...
    setCarregando(true);
    try {
      const dados = await unidadeCentralService.buscarUnidadeCompleta(codigoUnidade);
      if (dados) {
  ...
  const exportarDados = async () => {
    ...
    try {
      const blob = await unidadeCentralService.exportarDadosUnidade(dadosUnidade.unidade.codigo_unidade);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `unidade-${dadosUnidade.unidade.codigo_unidade}-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'escalonado': return 'bg-purple-100 text-purple-800';
  ...
  const getTipoColor = (tipo: string) => {
    switch (tipo) {
      case 'royalty': return 'bg-blue-100 text-blue-800';
      case 'insumo': return 'bg-green-100 text-green-800';
      case 'taxa_marketing': return 'bg-purple-100 text-purple-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Painel Central da Unidade</h1>
              <p className="text-gray-600">Visão 360° completa da unidade franqueada</p>
            </div>
          </div>
        ...
        {!dadosUnidade && (
          <div className="max-w-md mx-auto text-center">
            <div className="mb-6">
              ...
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Buscar Unidade</h2>
              <p className="text-gray-600">Digite o código da unidade para visualizar informações completas</p>
            </div>
          ...
          <div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6 mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">{dadosUnidade.unidade.nome_unidade}</h2>
                  <p className="text-gray-600">Código: {dadosUnidade.unidade.codigo_unidade} | CNPJ: {dadosUnidade.unidade.cnpj}</p>
                  <p className="text-gray-600">Franqueado: {dadosUnidade.unidade.nome_franqueado_responsavel}</p>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-red-600">{formatarMoeda(dadosUnidade.dashboard.total_em_aberto)}</div>
            ...
            {abaSelecionada === 'dados' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => editandoDados ? salvarDadosCadastrais() : setEditandoDados(true)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setDadosEdicao({...dadosEdicao, nome_unidade: e.target.value})}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Código Oficial</label>
                    <input
                      type="text"
                      value={dadosEdicao.codigo_oficial_franquia || ''}
                      onChange={(e) => setDadosEdicao({...dadosEdicao, codigo_oficial_franquia: e.target.value})}
                      disabled={!editandoDados}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Franqueado Responsável</label>
                    <input
                      type="text"
                      value={dadosEdicao.nome_franqueado_responsavel || ''}
                      onChange={(e) => setDadosEdicao({...dadosEdicao, nome_franqueado_responsavel: e.target.value})}
                      disabled={!editandoDados}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setDadosEdicao({...dadosEdicao, whatsapp_comercial: e.target.value})}
                      disabled={!editandoDados}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setDadosEdicao({...dadosEdicao, email_comercial: e.target.value})}
                      disabled={!editandoDados}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setDadosEdicao({...dadosEdicao, endereco_completo: e.target.value})}
                    disabled={!editandoDados}
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
                  />
            ...
            {abaSelecionada === 'cobrancas' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-800">Cobranças da Unidade</h3>
                  <div className="flex space-x-3">
                    <select
                      value={filtros.status_cobranca || ''}
                      onChange={(e) => setFiltros({...filtros, status_cobranca: e.target.value})}
                      ...
                      {dadosUnidade.cobrancas.map((cobranca) => (
                        <tr key={cobranca.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getTipoColor(cobranca.tipo)}`}>
            ...
            {abaSelecionada === 'agenda' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-800">Agenda e Reuniões</h3>
                  <button
            ...
            {abaSelecionada === 'comunicacoes' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-800">Comunicações</h3>
                  <div className="flex space-x-3">
                    <button
            ...
            {abaSelecionada === 'dashboard' && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-800">Dashboard da Unidade</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="bg-yellow-50 rounded-lg p-4">
                    <div className="text-2xl font-bold text-yellow-600">
                      {dadosUnidade.dashboard.percentual_inadimplencia.toFixed(1)}%
                    </div>
                    <div className="text-sm text-yellow-800">% Inadimplência</div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <h4 className="font-semibold text-gray-800 mb-4">Tendência de Regularização (6 meses)</h4>
                    <div className="space-y-2">
                      {dadosUnidade.dashboard.tendencia_regularizacao.map((valor, index) => (
                        <div key={index} className="flex items-center">
                          ...
                          <div className="w-12 text-sm text-gray-800">{valor.toFixed(0)}%</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <h4 className="font-semibold text-gray-800 mb-4">Resumo de Ações</h4>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Acordos Firmados:</span>
                        <span className="font-medium">{dadosUnidade.dashboard.acordos_firmados}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Cobranças Jurídico:</span>
                        <span className="font-medium text-red-600">{dadosUnidade.dashboard.cobrancas_juridico}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Reuniões Realizadas:</span>
                        <span className="font-medium">{dadosUnidade.dashboard.reunioes_mes}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {abaSelecionada === 'vinculos' && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-800">Vínculos do Franqueado</h3>
                
                <div className="bg-blue-50 rounded-lg p-6">
                  <h4 className="font-semibold text-blue-800 mb-4">
                    Franqueado Principal: {dadosUnidade.vinculos.franqueado_principal}
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{dadosUnidade.vinculos.total_unidades}</div>
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-sm text-red-800">Valor Total do Grupo</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {dadosUnidade.vinculos.outras_unidades.filter(u => u.status === 'ativa').length}
                      </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {dadosUnidade.vinculos.outras_unidades.map((unidade) => (
                        <tr key={unidade.codigo_unidade} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(unidade.status)}`}>
      ...
      {modalAberto === 'notificacao' && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold">Gerar Notificação Formal</h3>
              <button onClick={fecharModal} className="text-gray-500 hover:text-gray-700">✕</button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="notificacao_vencimento">Notificação de Vencimento</option>
                  <option value="notificacao_quebra_acordo">Quebra de Acordo</option>
                  <option value="carta_encerramento">Carta de Encerramento</option>
                </select>
              </div>
      ...
      {modalAberto === 'escalonamento' && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Motivo do Escalonamento</label>
                <textarea
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Valor Envolvido</label>
                <input
                  type="number"
                  step="0.01"
                  value={formData.valor_envolvido || 0}
                  onChange={(e) => setFormData({...formData, valor_envolvido: parseFloat(e.target.value) || 0})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
