...
import { 
  FileText, Plus, Edit, Trash2, Eye, Send, Settings, 
  Download, Filter, RefreshCw, Zap, AlertTriangle, 
  CheckCircle, Clock, User, Calendar, DollarSign,
  MessageSquare, Mail, Phone, Target, Bell, Save, X
} from 'lucide-react';
import { TemplatesJuridicosService } from '../services/templatesJuridicosService';
import { TemplateJuridico, GatilhoAutomatico, FiltrosTemplates, EstatisticasTemplates, HistoricoDisparo } from '../types/templatesJuridicos';

export function TemplatesJuridicos() {
  const [abaSelecionada, setAbaSelecionada] = useState<'templates' | 'gatilhos' | 'historico' | 'configuracoes'>('templates');
  ...
  const [modalAberto, setModalAberto] = useState<'criar-template' | 'editar-template' | 'criar-gatilho' | 'visualizar-historico' | null>(null);
  const [itemSelecionado, setItemSelecionado] = useState<any>(null);
  const [formData, setFormData] = useState<any>({});
  const [processando, setProcessando] = useState(false);
  const [estatisticas, setEstatisticas] = useState<EstatisticasTemplates | null>(null);
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (abaSelecionada === 'historico') {
        const historicoData = await templatesService.buscarHistoricoDisparos(filtros);
  ...
  const exportarDados = async () => {
    try {
      ...
      a.href = url;
      a.download = `templates-juridicos-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const getTipoDebitoColor = (tipo: string) => {
    switch (tipo) {
      ...
      case 'insumo': return 'bg-purple-100 text-purple-800';
  ...
  const getCategoriaIcon = (categoria: string) => {
    switch (categoria) {
      case 'notificacao': return <Bell className="w-4 h-4 text-blue-600" />;
      case 'advertencia': return <AlertTriangle className="w-4 h-4 text-yellow-600" />;
      case 'proposta_acordo': return <FileText className="w-4 h-4 text-green-600" />;
      case 'intimacao_juridica': return <Target className="w-4 h-4 text-red-600" />;
  ...
  const getCanalIcon = (canal: string) => {
    switch (canal) {
      ...
      case 'painel': return <User className="w-4 h-4 text-purple-600" />;
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Templates Jurídicos e Gatilhos Automáticos</h1>
              <p className="text-gray-600">Gestão de comunicações jurídicas por tipo de débito</p>
            </div>
          </div>
        ...
        {estatisticas && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-600">{estatisticas.total_templates}</div>
              <div className="text-sm text-blue-800">Templates Ativos</div>
            </div>
            <div className="bg-purple-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-purple-600">{estatisticas.gatilhos_ativos}</div>
              <div className="text-sm text-purple-800">Gatilhos Ativos</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-green-600">{estatisticas.disparos_mes}</div>
              <div className="text-sm text-green-800">Disparos Este Mês</div>
            </div>
            <div className="bg-yellow-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-yellow-600">{estatisticas.taxa_resposta.toFixed(1)}%</div>
                ...
                <div key={template.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        <h3 className="text-lg font-semibold text-gray-800">{template.nome}</h3>
                        <div className="flex items-center space-x-3 mt-1">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getTipoDebitoColor(template.tipo_debito)}`}>
                            {template.tipo_debito.toUpperCase()}
                          </span>
                          <span className="text-sm text-gray-600">{template.categoria.replace('_', ' ').toUpperCase()}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-sm text-gray-600">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2">Canal: {template.canal_envio}</span>
                    </div>
                    <div className="flex items-center">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button
                      onClick={() => ativarDesativarTemplate(template.id!, !template.ativo)}
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 mb-6">
              <div className="flex items-center">
                ...
                <div>
                  <h4 className="font-semibold text-yellow-800">Gatilhos Automáticos Configurados</h4>
                  <ul className="text-yellow-700 text-sm mt-2 space-y-1">
                    <li>• 2+ boletos vencidos do mesmo tipo → Advertência automática</li>
            ...
            {gatilhos.length === 0 ? (
              ...
              gatilhos.map((gatilho) => (
                <div key={gatilho.id} className="border border-gray-200 rounded-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-800">{gatilho.nome}</h3>
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      gatilho.ativo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p><strong>Condições:</strong> {gatilho.condicoes.join(', ')}</p>
                    <p><strong>Template:</strong> {gatilho.template_nome}</p>
                    <p><strong>Prioridade:</strong> {gatilho.prioridade}</p>
                    <p><strong>Execuções:</strong> {gatilho.total_execucoes || 0}</p>
                  </div>
                </div>
              ))
            )}
                ...
                {historico.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{item.unidade_nome}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="ml-2 text-sm text-gray-900">{item.canal_utilizado}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                        item.visualizado ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                      }`}>
        ...
        {abaSelecionada === 'configuracoes' && (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800">Configurações de Templates e Gatilhos</h3>
            
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
              <h4 className="font-semibold text-blue-800 mb-4">Variáveis Disponíveis para Templates</h4>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                {[
                  ...
                  <code key={variavel} className="bg-blue-100 text-blue-800 px-2 py-1 rounded">
                    {variavel}
                  </code>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h4 className="font-semibold text-gray-800 mb-4">Permissões de Acesso</h4>
      ...
      {(modalAberto === 'criar-template' || modalAberto === 'editar-template') && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold">
                {modalAberto === 'criar-template' ? 'Novo Template Jurídico' : 'Editar Template'}
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({...formData, tipo_debito: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option value="royalty">Royalty</option>
                  <option value="aluguel">Aluguel</option>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({...formData, prazo_resposta_dias: parseInt(e.target.value)})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                />
              </div>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                onChange={(e) => setFormData({...formData, corpo_mensagem: e.target.value})}
                rows={8}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                placeholder="Digite o conteúdo do template usando as variáveis disponíveis..."
              />
              <p className="text-sm text-gray-500 mt-2">
                Use variáveis como {{nome_unidade}}, {{valor_total_em_aberto}}, {{dias_em_atraso}} etc.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="gatilho_ativo" className="ml-2 text-sm font-medium text-gray-700">
                  Gatilho ativo
                </label>
              </div>
            </div>
