...
import { 
  FileText, Download, Eye, Plus, Filter, Calendar, User, 
  AlertTriangle, CheckCircle, Printer, Mail, Upload, Trash2
} from 'lucide-react';
import { DocumentosService } from '../services/documentosService';
import { DocumentoCobranca, FiltrosDocumentos, EstatisticasDocumentos, ChecklistDocumentos } from '../types/documentos';
...
export function GeradorDocumentos({ tituloId, cnpj, showUploadButton = true }: GeradorDocumentosProps) {
  const [documentos, setDocumentos] = useState<any[]>([]);
  const [carregando, setCarregando] = useState(true);
  const [filtros, setFiltros] = useState<FiltrosDocumentos>({});
  const [modalAberto, setModalAberto] = useState<'upload' | 'visualizar' | 'checklist' | null>(null);
  ...
  const [estatisticas, setEstatisticas] = useState<EstatisticasDocumentos | null>(null);
  ...
  const carregarChecklist = async () => {
    if (!tituloId) return;
    try {
      const checklistData = await documentosService.gerarChecklistDocumentos(tituloId);
  ...
  const uploadDocumento = async () => {
    ...
    try {
      ...
      const dadosDocumento = {
        ...
        codigo_unidade: (cobranca as any).unidades_franqueadas?.codigo_unidade || '',
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-gray-600">
                {tituloId ? 'Documentos da cobrança específica' : 
                 cnpj ? `Documentos da unidade ${cnpj}` : 
        ...
        {estatisticas && !tituloId && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-600">{estatisticas.total_documentos}</div>
              <div className="text-sm text-blue-800">Total de Documentos</div>
            </div>
            <div className="bg-yellow-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-yellow-600">{estatisticas.documentos_pendentes}</div>
              <div className="text-sm text-yellow-800">Documentos Pendentes</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-green-600">{estatisticas.uploads_mes_atual}</div>
              <div className="text-sm text-green-800">Uploads Este Mês</div>
            </div>
            <div className="bg-purple-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-purple-600">{estatisticas.tamanho_total_mb.toFixed(1)} MB</div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-6 mb-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-800">
                Completude dos Documentos - {checklist.status_cobranca.toUpperCase()}
              </h3>
              <div className="flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {checklist.documentos_obrigatorios.map((doc, index) => (
                <div key={index} className={`flex items-center p-3 rounded-lg ${
                  doc.presente ? 'bg-green-100 border border-green-200' : 'bg-red-100 border border-red-200'
                }`}>
                  {doc.presente ? (
                  ...
                  <div>
                    <div className={`font-medium ${doc.presente ? 'text-green-800' : 'text-red-800'}`}>
                      {getTipoDocumentoLabel(doc.tipo)}
                    </div>
                    <div className={`text-sm ${doc.presente ? 'text-green-600' : 'text-red-600'}`}>
                      {doc.descricao}
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              >
                ...
                <option value="notificacao_institucional">Notificação Institucional</option>
              ...
              <input
                ...
                onChange={(e) => setFiltros({...filtros, usuario_responsavel: e.target.value})}
                placeholder="Usuário"
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              />
            </div>
        ...
        <div className="space-y-4">
          {carregando ? (
            ...
            documentos.map((documento) => (
              <div key={documento.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600">
                        ...
                        <span>{formatarTamanho(documento.tamanho_arquivo)}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${getTipoColor(documento.tipo_documento)}`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormUpload({...formUpload, tipo_documento: e.target.value})}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setFormUpload({...formUpload, observacoes: e.target.value})}
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => setFormUpload({...formUpload, obrigatorio: e.target.checked})}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="obrigatorio" className="ml-2 text-sm font-medium text-gray-700">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">{documentoSelecionado.nome_arquivo}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Tipo</label>
                  <p className="text-gray-900">{getTipoDocumentoLabel(documentoSelecionado.tipo_documento)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Tamanho</label>
                  <p className="text-gray-900">{formatarTamanho(documentoSelecionado.tamanho_arquivo)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Data Upload</label>
                  <p className="text-gray-900">{formatarData(documentoSelecionado.data_upload)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Usuário</label>
                  <p className="text-gray-900">{documentoSelecionado.usuario_responsavel}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Status Cobrança</label>
                  <p className="text-gray-900">{documentoSelecionado.status_cobranca_vinculado}</p>
                </div>
              </div>
              ...
              {documentoSelecionado.observacoes && (
                <div>
                  ...
                  <p className="text-gray-900">{documentoSelecionado.observacoes}</p>
                </div>
              )}
