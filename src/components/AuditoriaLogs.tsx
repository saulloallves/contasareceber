...
export function AuditoriaLogs() {
  const [abaSelecionada, setAbaSelecionada] = useState<
    "logs" | "configuracoes" | "alertas"
  >("logs");
  const [logs, setLogs] = useState<LogAuditoria[]>([]);
  const [carregando, setCarregando] = useState(true);
  const [filtros, setFiltros] = useState<FiltrosAuditoria>({});
  const [estatisticas, setEstatisticas] =
    useState<EstatisticasAuditoria | null>(null);
  const [configuracao, setConfiguracao] =
    useState<ConfiguracaoAuditoria | null>(null);
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Rastreamento completo de ações e controle de acesso
              </p>
            </div>
          </div>
                      ...
                      <tr key={log.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <span
                              className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${getAcaoColor(
                                log.acao
                              )}`}
                            >
          ...
          <div className="space-y-6">
            ...
            {alertas.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  Nenhum alerta de auditoria ativo
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {alertas.map((alerta) => (
                  <div
                    key={alerta.id}
                    className={`border rounded-lg p-6 ${getRiscoColor(
                      alerta.nivel_risco
                    )}`}
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${getRiscoColor(
                          alerta.nivel_risco
                        )}`}
                      >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                      <div>
                        ...
                        {formatarData(alerta.data_deteccao)}
                      </div>
                      <div>
                        <span className="font-medium">Usuário:</span>{" "}
                        {alerta.usuario_envolvido}
                      </div>
                      <div>
                        <span className="font-medium">Entidade:</span>{" "}
                        {alerta.entidade_afetada}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  {Object.entries(configuracao.acoes_registradas).map(
                    ([acao, ativo]) => (
                      <div
                        key={acao}
