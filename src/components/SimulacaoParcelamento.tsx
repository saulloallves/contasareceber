...
import {
  Calculator, Search, Plus, Minus, Calendar, DollarSign, FileText,
  CheckCircle, AlertTriangle, RefreshCw, Building2, CreditCard, ArrowRight, ArrowLeft,
} from "lucide-react";
...
export function SimulacaoParcelamento() {
  // Estados principais
  const [etapaAtual, setEtapaAtual] = useState<'busca' | 'selecao' | 'simulacao' | 'proposta'>('busca');
  ...
  const [valorEntrada, setValorEntrada] = useState<number | undefined>(undefined);
  ...
  const simularParcelamento = async () => {
    ...
    try {
      ...
      setErro(error instanceof Error ? error.message : "Erro ao simular parcelamento");
    } finally {
  ...
  const gerarProposta = async () => {
    ...
    try {
      ...
      setErro(error instanceof Error ? error.message : "Erro ao gerar proposta");
    } finally {
      setProcessando(false);
    }
  };
  ...
  const enviarWhatsApp = async () => {
    ...
    setProcessando(true);
    try {
      const sucesso = await simulacaoService.enviarPropostaWhatsApp(proposta.id);
      if (sucesso) {
  ...
  // Calcular totais das cobranças selecionadas
  const totaisSelecionadas = cobrancasSelecionadas.reduce((acc, id) => {
    const cobranca = cobrancasDisponiveis.find(c => c.id === id);
    if (cobranca) {
      acc.valorOriginal += cobranca.valor_original || 0;
      acc.valorAtualizado += cobranca.valor_atualizado || cobranca.valor_original || 0;
      acc.quantidade += 1;
    }
    return acc;
  }, { valorOriginal: 0, valorAtualizado: 0, quantidade: 0 });

  // Obter dados da unidade (primeira cobrança)
  const dadosUnidade = cobrancasDisponiveis.length > 0 ? {
    documento: cobrancasDisponiveis[0].cnpj || cobrancasDisponiveis[0].cpf,
      ...
      <div className="bg-white rounded-xl shadow-md p-8 border border-gray-100">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">Simulação de Parcelamento <span>💳</span></h1>
              <p className="text-gray-500 text-sm">
                {etapaAtual === 'busca' && "Selecione o cliente para iniciar o parcelamento"}
                {etapaAtual === 'selecao' && "Selecione as cobranças para parcelar"}
                {etapaAtual === 'simulacao' && "Configure e simule o parcelamento"}
            ...
            {["Busca", "Seleção", "Simulação", "Proposta"].map((etapa, idx) => (
              <div key={etapa} className="flex items-center space-x-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 ${
                  idx === ["busca", "selecao", "simulacao", "proposta"].indexOf(etapaAtual)
                ...
                <span className="text-xs font-medium text-gray-700">{etapa}</span>
          ...
          <div className="space-y-6">
            <div className="border border-gray-200 rounded-lg p-6 bg-gray-50">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Selecione um Cliente para Parcelamento</h3>
              </div>
              {carregando ? (
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-800">{dadosUnidade.cliente}</h3>
                      <p className="text-gray-600">
                        {dadosUnidade.tipo_documento}: {formatarCNPJCPF(dadosUnidade.documento)}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-600">Cobranças Disponíveis</p>
                    <p className="text-2xl font-bold text-[#ff9923]">{cobrancasDisponiveis.length}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  Selecione as Cobranças ({cobrancasSelecionadas.length} de {cobrancasDisponiveis.length})
                </h3>
              ...
              {cobrancasSelecionadas.length > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="text-sm text-green-800">
                    <p className="font-medium">Selecionadas: {totaisSelecionadas.quantidade} cobrança(s)</p>
                    <p>Valor Original: {formatarMoeda(totaisSelecionadas.valorOriginal)}</p>
                    <p>Valor Atualizado: {formatarMoeda(totaisSelecionadas.valorAtualizado)}</p>
                  </div>
                </div>
              )}
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${
                          selecionada ? 'border-blue-500 bg-blue-500' : 'border-gray-300'
                        }`}>
                          {selecionada && <CheckCircle className="w-4 h-4 text-white" />}
                        </div>
                        
                        <div>
                          <p className="font-medium text-gray-800">{cobranca.descricao || cobranca.tipo_cobranca}</p>
        ...
        {etapaAtual === 'simulacao' && (
          <div className="space-y-6">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-purple-800 mb-4">Resumo das Cobranças Selecionadas</h3>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Quantidade</p>
                  <p className="text-2xl font-bold text-purple-600">{totaisSelecionadas.quantidade}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Valor Original</p>
                  <p className="text-xl font-bold text-gray-800">{formatarMoeda(totaisSelecionadas.valorOriginal)}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Valor Atualizado</p>
                  <p className="text-xl font-bold text-red-600">{formatarMoeda(totaisSelecionadas.valorAtualizado)}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Cliente</p>
                  <p className="text-lg font-bold text-gray-800">{dadosUnidade?.cliente}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="flex items-center space-x-3">
                  <button
                    onClick={() => setQuantidadeParcelas(Math.max(3, quantidadeParcelas - 1))}
                  ...
                  <input
                    ...
                    onChange={(e) => setQuantidadeParcelas(parseInt(e.target.value) || 3)}
                    className="w-20 text-center px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                  />
                  <button
                    onClick={() => setQuantidadeParcelas(Math.min(42, quantidadeParcelas + 1))}
                    className="p-2 bg-gray-100 rounded-lg hover:bg-gray-200"
                  >
                    <Plus className="w-4 h-4" />
                  </button>
                </div>
                <p className="text-xs text-gray-500 mt-2">Entre 3 e 42 parcelas</p>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <input
                  ...
                  step="0.01"
                  value={valorEntrada || ''}
                  onChange={(e) => setValorEntrada(parseFloat(e.target.value) || undefined)}
                  placeholder="Deixe vazio para entrada mínima"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                />
                <p className="text-xs text-gray-500 mt-2">
                  Mínimo: {formatarMoeda(totaisSelecionadas.valorAtualizado * 0.2)}
                </p>
              </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-lg font-semibold text-gray-800">Pronto para Simular?</h4>
                  <p className="text-gray-600">
                    {totaisSelecionadas.quantidade} cobrança(s) • {quantidadeParcelas} parcelas • 
                    Valor: {formatarMoeda(totaisSelecionadas.valorAtualizado)}
                  </p>
                </div>
                <button
          ...
          <div className="space-y-6 mt-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Valor Original</p>
                  <p className="text-xl font-bold text-gray-800">{formatarMoeda(simulacao.valor_original)}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Valor Atualizado</p>
                  <p className="text-xl font-bold text-red-600">{formatarMoeda(simulacao.valor_atualizado)}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Entrada</p>
                  <p className="text-xl font-bold text-green-600">{formatarMoeda(simulacao.valor_entrada || 0)}</p>
                </div>
                <div className="bg-white rounded-lg p-4">
                  <p className="text-sm text-gray-600">Total do Parcelamento</p>
                  <p className="text-xl font-bold text-[#ff9923]">{formatarMoeda(simulacao.valor_total_parcelamento)}</p>
                </div>
              </div>

              {/* Cronograma de Parcelas */}
              <div className="bg-white rounded-lg p-4">
                <h4 className="font-medium text-gray-800 mb-3">Cronograma de Pagamentos:</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {simulacao.valor_entrada > 0 && (
                    <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                      <span className="font-medium text-green-800">Entrada</span>
                      <div className="text-right">
                        <p className="font-bold text-green-600">{formatarMoeda(simulacao.valor_entrada)}</p>
                        <p className="text-xs text-gray-500">{formatarData(simulacao.data_primeira_parcela)}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-2">
                    {simulacao.parcelas.map((parcela: any, index: number) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                        <span className="font-medium text-blue-800">Parcela {parcela.numero}</span>
                        <div className="text-right">
                          <p className="font-bold text-[#ff9923]">{formatarMoeda(parcela.valor)}</p>
                          <p className="text-xs text-gray-500">{formatarData(parcela.data_vencimento)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-6">
                <h4 className="font-medium text-yellow-800 mb-2">Detalhes dos Encargos:</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">Multa:</span>
                    <span className="font-medium text-yellow-700 ml-2">{simulacao.percentual_multa}%</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Juros de Mora:</span>
                    <span className="font-medium text-yellow-700 ml-2">{simulacao.percentual_juros_mora}%</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Economia na Entrada:</span>
                    <span className="font-medium text-green-700 ml-2">{formatarMoeda(simulacao.economia_total || 0)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-medium text-gray-800 mb-3">Informações da Proposta:</h4>
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Cliente:</span> {dadosUnidade?.cliente}</p>
                    <p><span className="font-medium">Documento:</span> {formatarCNPJCPF(dadosUnidade?.documento || '')}</p>
                    <p><span className="font-medium">Cobranças:</span> {totaisSelecionadas.quantidade}</p>
                    <p><span className="font-medium">Parcelas:</span> {simulacao.quantidade_parcelamento}x</p>
                    <p><span className="font-medium">Valor Total:</span> {formatarMoeda(simulacao.valor_total_parcelamento)}</p>
                    <p><span className="font-medium">Válida até:</span> {formatarData(proposta.data_expiracao)}</p>
                  </div>
                </div>
                <div>
                  <h4 className="font-medium text-gray-800 mb-3">Preview da Mensagem:</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <p className="text-sm text-gray-500 mb-4">
                  Envia a proposta diretamente para o WhatsApp do cliente via integração n8n.
                </p>
                <button
