...
export function GestaoBloqueios() {
  ...
  const [modalAberto, setModalAberto] = useState<
    "detalhes" | "verificar" | "configurar" | null
  >(null);
  const [bloqueioSelecionado, setBloqueioSelecionado] = useState<any>(null);
  const [processando, setProcessando] = useState(false);
  const [estatisticas, setEstatisticas] = useState<EstatisticasBloqueio | null>(
    null
  );
  ...
  const verificarCriteriosIndividual = async () => {
    ...
    setProcessando(true);
    try {
      const criterio = await bloqueioService.verificarCriteriosBloqueio(
        cnpjVerificacao
      );
  ...
  const executarVerificacaoLote = async () => {
    if (
      !confirm(
        "Deseja executar a verificação em lote de todas as unidades? Esta operação pode demorar alguns minutos."
      )
    ) {
      return;
    }
          ...
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">
                Sistema de Bloqueio Automatizado
              </h1>
              <p className="text-gray-600">
                Controle de acessos e benefícios por inadimplência
              </p>
            </div>
          </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <select
              ...
              onChange={(e) =>
                setFiltros({ ...filtros, status_bloqueio: e.target.value })
              }
            ...
            <select
              ...
              onChange={(e) =>
                setFiltros({ ...filtros, motivo_bloqueio: e.target.value })
              }
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500"
            >
              ...
              {carregando ? (
                ...
                bloqueios.map((bloqueio) => (
                  <tr key={bloqueio.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">
                          {bloqueio.unidades_franqueadas?.nome_franqueado ||
                            "N/A"}
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getMotivoColor(
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {bloqueio.data_bloqueio
                          ? formatarData(bloqueio.data_bloqueio)
                          : "-"}
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        {bloqueio.status_bloqueio === "ativo" && (
                          <button
                            onClick={() =>
                              executarDesbloqueioManual(bloqueio.cnpj_unidade)
                            }
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-semibold">
                Verificar Critérios de Bloqueio
              </h3>
              <button
                onClick={fecharModal}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                <p className="text-sm text-orange-800">
                  <AlertTriangle className="w-4 h-4 inline mr-1" />O sistema
                  verificará automaticamente todos os critérios de bloqueio para
                  esta unidade.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        {bloqueioSelecionado.unidades_franqueadas.cidade}/
                        {bloqueioSelecionado.unidades_franqueadas.estado}
                      </p>
                    </div>
                    <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {bloqueioSelecionado.acessos_bloqueados.map(
                    (acesso: string) => (
                      <div
                        key={acesso}
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-red-600">
                          {bloqueioSelecionado.motivo_bloqueio}
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-gray-500">
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-green-600">
                            {bloqueioSelecionado.observacoes}
                          </div>
                        </div>
                      </div>
                      <div className="text-sm text-gray-500">
